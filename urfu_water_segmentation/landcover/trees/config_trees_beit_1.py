# План правок:
# 1. Изменить функцию потерь: Done
# 	Выбрать с чем комбинировать CE: Dice или Lovasz-Softmax
# 	Посчитать веса для деревьев и фона на train
# 2. Аугментация снимков: Done
# 	Добавить RandomRotate и VerticalFlip
# 3. Выбрать другой base_lr Done
# 4. Добавить `clip_grad` в `optim_wrapper` Done
...
train_pipeline = [
    ...
    dict(type='RandomResize', scale=(2048, 512), ratio_range=(0.75, 1.5), keep_ratio=True),
    ...
    dict(type='RandomRotate', prob=0.5, degree=180, pad_val=0, seg_pad_val=ignore_index),
    dict(type='RandomRotate', prob=0.5, degree=270, pad_val=0, seg_pad_val=ignore_index),
